...
public final class AlbumArtCache {
    ...
    public void fetch(final String artUrl, final FetchListener listener) {
        ...
        /*
        java.io.FileNotFoundException，其实文件是存在的，把地址拷贝到手机浏览器都可以直接下载的，但为嘛不能下载呢。
        原因是。。。。汉字问题。浏览器对汉字进行了解析为AsciI字符。可以对该String进行如下操作即可。
        String icon = new URI(icon).toASCIIString();
        此问题在android7.1没有出现，在开发板上才出现这种问题，可见高版本android系统对uri自动进行了某些处理
         */
        new AsyncTask<Void, Void, Bitmap[]>() {
            ...
            protected Bitmap[] doInBackground(Void[] objects) {
                ...
                try {
                    ...
                    try {
                        asciiUrl = new URI(artUrl).toASCIIString();
                    } catch (URISyntaxException e) {
                        e.printStackTrace();
                    }
                    Bitmap bitmap = BitmapHelper.fetchAndRescaleBitmap(asciiUrl,
